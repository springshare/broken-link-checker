...
class CLI
{
	constructor()
	{
		...
		filterLevel += "  1: 0 + media, frames, meta refreshes\n";
...
function logResults(data)
{
	...
	while (nextIsReady)
	{
		const result = data.page.results[data.page.currentIndex];
		
		if (result !== undefined)
		{
			const done = data.page.done===true && data.page.currentIndex>=data.page.results.length-1;
			
			const output = logResult(result, done);
...
function run(url, checkerOptions, logOptions)
{
	...
	// In case first page doesn't call "html" event
	resetPageData(data);
	...
	{
		instance = new blc.HtmlUrlChecker(checkerOptions);
	}
	else
	{
		instance = new blc.SiteChecker(checkerOptions);
	}
