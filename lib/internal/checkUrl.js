...
function checkUrl(link, baseUrl, cache, options, retry)
{
	...
	{
		response = simpleResponse(response, link.url.resolved);

		if (response.statusCode !== 200 && options.requestMethod==="HEAD" && options.retry405Head===true && retry!==405)
		{
	...
	{
        // Proxies tend to have malformed HEADs, retry with GET.
        if ((error.message === 'Error: Parse Error' || error.message === 'Error: socket hang up' || error.message.startsWith('Error: Exceeded maxRedirects.')) && retry !== 405) {
            return checkUrl(link, baseUrl, cache, options, 405);
        } else {
            return error;
        }
