...
{
	return isString(input)===false && input.broken_link_checker===true;
}
...
{
	link.url.redirected = specurl.parse(url);
...
function relation(link)
{
	...
	else
	{
		...
		let relation = specurl.relation(link.base.resolved, url);
		
		link.internal = relation >= specurl.relation.AUTH;
		link.samePage = relation >= specurl.relation.PATH;
	}
...
function resolve(link, url, base)
{
	...
	base = isString(base)===true ? specurl.parse(base) : specurl.clone(base);
	...
	else
	{
		...
		link.url.resolved = specurl.parse(url);
	}
	...
	if (base !== null)
	{
		...
		link.base.rebased = specurl.resolve(link.base.resolved, link.html.base) || link.base.resolved;
	}
	else
	{
		link.base.rebased = specurl.parse(link.html.base);
	}
	...
	if (link.base.rebased !== null)
	{
		...
		{
			link.url.resolved = specurl.resolve(link.base.resolved, url);
			link.url.rebased  = specurl.resolve(link.base.rebased, url);
		}
		else
		{
